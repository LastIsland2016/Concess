...
AddEventHandler('MenuConcessCatalogue', function()
    Catalogue()
end)
...
function MenuAchat(name, model, price, category, stock, label)
    ...
    lib.callback('Sacario:concess_auto_verif', false, function(result)
        if result then 
            for k, v in pairs(result) do
                                    ...
                                    if stock >= 1 then 
                                        ...
                                        if input then 
                                            ...
                                            if alert == 'confirm' then
                                                ...
                                                lib.callback('Sacario:concess_verif_money', false, function(cb)
                                                    if cb then 
                                                        ESX.Game.SpawnVehicle(model, Config.Catalogue.SpawnVehicle.coords, Config.Catalogue.SpawnVehicle.heading, function(vehicle)
                                                            ...
                                                            TriggerServerEvent('Sacario:concess_buy_car', name, price, properties, plate,stock, input[1], input[2])
                                                            TriggerServerEvent('Sacario:concess_add_historique', "Vente", "/", price, name)
                                                        end)
                                                    else 
                                                        lib.notify({
...
function MenuVeh(categories)
    local option = {}
    lib.callback('Sacario:concess_liste_veh_from_cat', false, function(result)
        if #result >= 1 then 
            for k, v in pairs(result) do
                    table.insert(option, {   
                        title = v.name,
...
function OpenPreview(model)
    ...
    ESX.Game.SpawnVehicle(model, Config.Preview.SpawnVehicle.coords,  Config.Preview.SpawnVehicle.heading, function(vehicle)
        SetVehicleNumberPlateText(vehicle, "Concess")
        cam()
                                    ...
                                    if input1 and input1[1] == 'particulier' and closestPlayer ~= nil then
                                        ...
                                        if input and input[3] ~= nil then 
                                            ...
                                            if alert == 'confirm' then
                                                local plate = GeneratePlate()
                                                local r, g, b = extractRGB(input[1])
                                                lib.callback('Sacario:concess_verif_money_vendeur', false, function(cb)
                                                    if cb then 
                                                        ESX.Game.SpawnVehicle(model,  Config.Catalogue.SpawnVehicle.coords,  Config.Catalogue.SpawnVehicle.heading, function(vehicle)
                                                            SetVehicleNumberPlateText(vehicle, plate)
                                                            SetVehicleCustomPrimaryColour(vehicle, r, g, b)
                                                            ...
                                                            TriggerServerEvent('Sacario:concess_buy_car_vendeur', name, price, properties, plate, stock, input[1], input[2], input[3])
                                                            TriggerServerEvent('Sacario:concess_add_historique', "Vente Automatique", "/", price, name)
                                                        end)
                                                    else 
                                                        lib.notify({
                                                            title = 'Achat Concessionnaire',
                                    ...
                                    elseif input1[1] == 'entreprise' then
                                        ...
                                        if input then
                                            ...
                                            if alert == 'confirm' then
                                                local plate = GeneratePlate()
                                                local r, g, b = extractRGB(input[1])
                                                lib.callback('Sacario:concess_verif_money_entreprise', false, function(cb)
                                                    if cb then 
                                                        ESX.Game.SpawnVehicle(model,  Config.Catalogue.SpawnVehicle.coords,  Config.Catalogue.SpawnVehicle.heading, function(vehicle)
                                                            SetVehicleNumberPlateText(vehicle, plate)
                                                            SetVehicleCustomPrimaryColour(vehicle, r, g, b)
                                                            ...
                                                            Wait(300)
                                                            SetPedIntoVehicle(PlayerPedId(), vehicle, -1)
                                                            TriggerServerEvent('Sacario:concess_buy_car_entreprise', name, price, properties, plate, stock, label, input[1], input[2])
                                                            TriggerServerEvent('Sacario:concess_add_historique', "Vente Automatique", "/", price, name)
                                                        end)
                                                    else 
                                                        lib.notify({
...
function IsPlateTaken(plate)
	...
	ESX.TriggerServerCallback('Sacario:concess_isPlateTaken', function(isPlateTaken)
