	...
	options = {
		{
            ...
            arrow = true,
            onSelect = function()
                TriggerServerEvent('sacario_concess:AnnonceOuverture')
            end
        },
        {
            title  = 'Fermeture',
...
AddEventHandler('MenuHistorique', function()
    MenuHistorique()
end)
...
function MenuHistorique()
    local option = {}
    lib.callback('Sacario:concess_historique', false, function(result)
        if result then
                ...
                table.insert(option, {
                    ...
                    onSelect = function()
                        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'concessMoto' and ESX.PlayerData.job.grade == 4 then
                            ...
                            if input then 
                                TriggerServerEvent('Sacario:concess_delete_historique', v.id)
                            end 
                        else 
                            lib.notify({
                            ...
                            lib.showContext('MenuHistorique')
                        end 
                    end 
                })
                lib.registerContext({
...
function MenuAutomatique()
    ...
    lib.callback('Sacario:concess_auto_verif', false,function(result)
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            onSelect = function()
                                ...
                                if input then 
                                    TriggerServerEvent('Sacario:concess_update_automatisation', input[1])
                                end 
                            end
                        }
        ...
        lib.showContext('MenuAutomatique')
    end)
end
