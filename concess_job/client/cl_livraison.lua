...
AddEventHandler('StartLivraison', function(stock, modela, price)
    info.stockVeh = stock
...
function StartLivraison()
    if CanStartLivraison then 
        SpawnPedLivraison()
    else 
        lib.notify({
            title = 'Livraison',
...
function SpawnPedLivraison()
	...
	SetVehicleNumberPlateText(VehiculeLivraison, "Luxury")
...
AddEventHandler('RecupVehicle', function()
    ...
    TriggerServerEvent('Sacario:LogsSend', 'Concessionnaire', 'Livraison', 'Le véhicule a été livré', Config.Webhook)
    lib.notify({
...
function RecupVehicle()
        ...
        while not CanStartLivraison do
    ...
    if IsControlJustReleased(1,51) then
        ...
        if IsPedInAnyVehicle(PlayerPedId()) then           
            if VehUse == VehiculeToDelivery then
                TriggerServerEvent('Sacario:concess_restock', info.stockVeh, info.modelVeh, info.priceVeh) 
                TriggerServerEvent('Sacario:LogsSend', 'Concessionnaire', 'Livraison', 'Le véhicule livré a été ajouté au stock !', Config.Webhook)
                TriggerServerEvent('Sacario:concess_add_historique', "Restock véhicule", info.priceVeh, "/", info.modelVeh)
