...
function GetVehicleSociety()
    lib.callback("Sacario:concess_VehiculeSociety",false, function(result)
...
function GetVehicleDispo()
    lib.callback('Sacario:concess_garage', false, function(result)
    ...
    if stored == 1 then
        if ESX.Game.IsSpawnPointClear(SpawnVehiculeZone, 5.0) then 
            ESX.Game.SpawnVehicle(json.decode(properties).model, Garage.SpawnVehicle.coords, Garage.SpawnVehicle.heading,function(vehicle)
                ...
                TriggerServerEvent('Sacario:concess_SaveSortie', plate)
        ...
        if IsPedInAnyVehicle(PlayerPedId()) then
            ...
            lib.callback("Sacario:concess_VehiculeSociety",false, function(result)
                ...
                if PlateVeh == Plate then
                    ...
                    TriggerServerEvent("Sacario:concess_SaveStatsEntrer", PlateVeh, 1, ESX.Game.GetVehicleProperties(VehUse))            
                    DeleteEntity(VehUse)
                    lib.notify({
                        id = 'notif_rentrer',
                        title = 'Garage Concessionnaire',
